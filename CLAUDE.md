...
### Target companies list (Notion-managed, Step 14)

A curated subset of companies worth adding by hand — not the full `discover_tokens()` seed
union (`TARGET_COMPANIES` ∪ every company ever scraped, hundreds of rows) — lives in its own
small Notion **database**, parallel to the Scratch Pad and Restricted Sponsorship Companies
databases: each row's title is a company name, plus `Greenhouse`/`Lever`/`Ashby` rich_text
columns and a `Last Checked` date. `python run.py --init` (→ `scripts/provision_notion.py`)
provisions it automatically alongside the other three, under the same "Careerpilot-ai" page,
and writes its id to `.env` — or create it by hand and set `NOTION_TARGET_COMPANIES_PAGE_ID`
(optional; the feature no-ops if unset).

This solves two gaps in the local-only setup: `config/profile.json`'s `target_companies` is a
plain array in a git-ignored file (can't be edited from a phone or a fresh CI checkout), and
`config/ats_tokens.json` (the discovered-token cache) is also git-ignored, so a GitHub Actions
run starts it empty every time and re-probes from zero instead of compounding across runs.
`discover_tokens()` (`scripts/sources.py`) seeds in `get_target_companies_from_notion()`
alongside its `companies` argument, and overlays `get_ats_tokens_from_notion()` onto the local
JSON cache before probing — **Notion is the source of truth for rows that exist there**, so a
token discovered for one of these companies survives a fresh checkout even with no local
`config/ats_tokens.json`. A fresh probe result for a Notion-seeded company is written back via
`upsert_ats_token_to_notion()` (create-or-update by title match), not just cached locally. The
broader auto-discovered tail (companies not in this curated list) keeps using the local-only
cache exactly as before this feature existed.
...
- `get_target_companies_from_notion()` / `get_ats_tokens_from_notion()` / `upsert_ats_token_to_notion()` — the Notion read/read/create-or-update helpers backing the curated target-companies list and persistent ATS-token store (see "Target companies list" above)

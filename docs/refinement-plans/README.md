...
| [`onboarding/forkable-setup.md`](onboarding/forkable-setup.md) | Phase 1 landed / Phase 2 open | ✅ Phase 1 (Notion): `--init` wizard + `scripts/provision_notion.py` (page + all three DBs) + env-sourced `NOTION_DB_ID` + hardened `--setup` — ⚠ needs the CI `NOTION_DB_ID` secret follow-up. ⏳ Phase 2 (identity): `config/profile.json`, untrack personal resume files, genericize owner references (Step 11) |

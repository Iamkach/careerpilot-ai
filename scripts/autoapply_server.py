...
────────────────────────────────────────────────────────────────────────────────
Spec: docs/backlog/step-15-application-prefill-extension.md — architecture/why, plus increment 0
      (scaffold), increment 1 (/plan, identify_job), increment 3a (/resume bytes), increment 4
      (/drafts), and increment 5 (/confirm-applied), all implemented in this one module.
...
WHAT THIS IS
    ...
    so a human standing on a live application form can get it pre-filled.
      ...
      POST /confirm-applied — the ONLY status write this bridge makes (step-15g). Sets
                             Status="Applied" on exactly one job, and only when the human
                             clicked a real button for it — see the "Applied invariant" note
                             below.
      POST /drafts        — one AI-drafted answer per free-text question (step-15f). A separate
                             route from /plan on purpose: one AI call per question is 10-30s and
                             would block every fill if bundled into /plan. Gated on
                             AUTOAPPLY_DRAFT_ESSAYS and a resolved page_id. Never writes
                             status/value on a field — only adds a `draft` key, so the fill
                             loop's `status == "ready"` predicate (step-15e) still structurally
                             excludes every drafted field with no special-casing.

APPLIED INVARIANT (step-15g) — reformulated, not broken
    `WRITABLE_STATUSES` (scripts/autoapply.py) excludes "Applied" on purpose: comparable
    auto-appliers are widely reported to mark jobs applied that were never submitted. That
    rationale names one permitted exception — "set by the human, by hand, after they click
    Submit" — it just assumed that hand was in Notion's UI. `/confirm-applied` gives it a
    second hand, gated identically hard:
      - the literal status is hard-coded in handle_confirm_applied(), never a request field;
      - the request body must carry confirmed_by == "human", else 400, no write attempted;
      - page_id must be a single string — a list is explicitly rejected, so "mark the backlog
        applied" is structurally never one call away;
      - the write goes through db_update_status_verified() (utils.py), which re-reads the page
        and reports failure rather than silently no-op'ing if Notion dropped the status;
      - Application Log always gets a "human-confirmed via extension" line, so every
        tool-mediated Applied write stays labelled and auditable.
    HUMAN_CONFIRMED_STATUS/CONFIRMABLE_STATUSES are asserted disjoint from WRITABLE_STATUSES by
    tests/test_autoapply_applied_confirmation.py — this route must never gain a second status it
    can write.
...
from scripts.autoapply import (
    ...
    draft_free_text_answers, _resume_text_for,
)
from scripts.autoapply_browser import accepts_docx
from scripts.render_docx import convert_docx_to_pdf
from scripts.utils import (
    db_get_jobs, db_get_ready_to_apply, db_update_status_verified, db_get_job_description, today,
)
from config.settings import RESUMES_DIR, AUTOAPPLY_DRAFT_ESSAYS
...
# step-15g — the ONE status this bridge is ever allowed to write, and only via
# handle_confirm_applied() below (never inferred from plan/fill data). Asserted disjoint from
# WRITABLE_STATUSES by tests/test_autoapply_applied_confirmation.py: those are Stage 7's own
# planning statuses, this is the human's, and the two must never overlap.
HUMAN_CONFIRMED_STATUS = "Applied"
CONFIRMABLE_STATUSES = {HUMAN_CONFIRMED_STATUS}
...
def build_drafts_response(payload: dict) -> dict:
    """Draft prose for every unresolved free-text field in the caller-supplied plan (the exact
    plan object the caller's own /plan call returned), in place, via
    draft_free_text_answers() — reused verbatim, so no answer logic is duplicated here.

    Requires a matched page_id: drafting needs the JD + resume text, and "no Notion match"
    should degrade to plain review_required, not a guess. Gated on AUTOAPPLY_DRAFT_ESSAYS,
    matching the settings-gated-feature pattern used elsewhere in this project (stage 2/7).
    """
    if not AUTOAPPLY_DRAFT_ESSAYS:
        return {"ok": False, "error": "drafting is disabled (AUTOAPPLY_DRAFT_ESSAYS=False)",
                "status": HTTPStatus.NOT_FOUND}

    page_id = payload.get("page_id")
    plan = payload.get("plan")
    if not page_id or not isinstance(plan, dict):
        return {"ok": False, "error": "page_id and plan are required",
    ...
    if job is None:
        ...
        job = next((c for c in pool if c.get("page_id") == page_id), None)
    if job is None:
        return {"ok": False, "error": "job not found", "status": HTTPStatus.NOT_FOUND}

    jd = db_get_job_description(page_id) or ""
    resume_text = _resume_text_for(job)
    drafted = draft_free_text_answers(plan, job, jd, resume_text)

    return {"ok": True, "drafted_count": drafted, "plan": plan}


# ── POST /confirm-applied ───────────────────────────────────────────────────────

def build_confirm_applied_response(payload: dict) -> dict:
    """The only status write this bridge makes. Returns {"ok": True, "page_id"} on success, or
    {"ok": False, "error", "status"} — the caller (do_POST) maps `status` to the HTTP code.

    Deliberately narrow, by design (see the module docstring's "Applied invariant" note):
      - confirmed_by must literally be "human" — no other value, no default;
      - page_id must be a single string — a list/missing value is rejected outright, never
        "confirm the whole candidate pool";
      - the status literal (HUMAN_CONFIRMED_STATUS) is never read from the payload.
    """
    if payload.get("confirmed_by") != "human":
        return {"ok": False, "error": "confirmed_by must be 'human'", "status": HTTPStatus.BAD_REQUEST}

    page_id = payload.get("page_id")
    if not isinstance(page_id, str) or not page_id:
        return {"ok": False, "error": "page_id must be a single non-empty string",
                "status": HTTPStatus.BAD_REQUEST}

    log_line = f"{today()} Applied — human-confirmed via extension"
    verified = db_update_status_verified(
        page_id, HUMAN_CONFIRMED_STATUS,
        {"date_applied": today(), "application_log": log_line},
    )
    if not verified:
        return {"ok": False, "error": "Notion did not apply Status='Applied' — add that option "
                                       "to the Status select by hand, then retry",
                "status": HTTPStatus.CONFLICT}

    reset_candidate_pool_cache()  # this job no longer belongs in the launcher's ready list
        ...
        def do_POST(self):
            ...
            if path == "/drafts":
                ...
                try:
                    ...
                    self._send_json(HTTPStatus.BAD_REQUEST, {"error": "invalid JSON body"})
                    return
                result = build_drafts_response(payload)
                if not result["ok"]:
                    self._send_json(result["status"], {"error": result["error"]})
                    return
                self._send_json(HTTPStatus.OK,
                                 {"ok": True, "drafted_count": result["drafted_count"],
                                  "plan": result["plan"]})
                return

            if path == "/confirm-applied":
                if not self._authed():
                    self._send_json(HTTPStatus.UNAUTHORIZED, {"error": "invalid or missing token"})
                    return
                length = int(self.headers.get("Content-Length", 0) or 0)
                ...
                try:
                    ...
                    self._send_json(HTTPStatus.BAD_REQUEST, {"error": "invalid JSON body"})
                    return
                result = build_confirm_applied_response(payload)
                if not result["ok"]:
                    self._send_json(result["status"], {"error": result["error"]})
                    return
                self._send_json(HTTPStatus.OK, {"ok": True, "page_id": result["page_id"]})
                return
